...
public class DoubleView extends View {
    ...
    private void drawScoreText(Canvas canvas) {
        ...
        //
        int bodyWidthScore = (int) (paint.measureText("" + game.score_1));
        ...
        backgroundRectangle.setBounds(2 * sXScore - eXScore, sYAll, sXScore, eYAll);
        backgroundRectangle.draw(canvas);
        paint.setTextSize(titleTextSize);
        paint.setColor(TEXT_BROWN);
        ...
        //
        backgroundRectangle.setBounds(sXScore - 200, sYAll, eXScore - 200, eYAll);
        backgroundRectangle.draw(canvas);
        paint.setTextSize(titleTextSize);
        paint.setColor(TEXT_BROWN);
    ...
    public void printAll() {
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                Tile currentTile = game.grid.getCellContent(yy, xx);
                String value;
                if (currentTile != null)
                    value = "" + currentTile.getValue();
                else
                    value = "null";
        ...
        for (int xx = 0; xx < game.numSquaresX; xx++) {
            for (int yy = 0; yy < game.numSquaresY; yy++) {
                ...
                if (currentTile != null) {
                    ...
                    int index;
                    if (value == 0) index = 0;
                    else if (value > 0) index = log2(value);
                    else index = log2(-value) + 11;
                    ;
    ...
    private int findValue(int xx) {
        if (xx == 0) return 0;
        else if (xx < 12) return (int) Math.pow(2, xx);
        else return (int) Math.pow(2, xx - 11);
    }
